...
bool tryDirectConnect(node* ps, node* pe_node, i)
{
  ROS_INFO("Attempting direct connect");
  NED_s pe = pe->p;
  node* start_of_line;
  if (ps->dontConnect) // then try one of the grand children
    start_of_line = findClosestNodeGChild(node* ps, pe->p);
  else
    start_of_line = ps;
  if (col_det_.checkLine(start_of_line->p, pe->p, path_clearance_))
  {
    if (start_of_line->parent == NULL) // then this is the start
    {
      float chi = ps->p.getChi(pe->p - start_of_line->p);
      if (col_det_.checkAfterWP(pe->p, chi)))
      {
        start_of_line->cost        = start_of_line->cost + (pe - start_of_line->p).norm();
    ...
    {
      fillet_s fil;
      bool fil_possible = fil.calculate(start_of_line->parent->p, start_of_line->p, pe->p, input_file_.turn_radius);
      if (fil_possible && col_det_.checkFillet(fil, clearance))
      {
        ...
        if (col_det_.checkAfterWP(pe->p, chi)))
        {
          start_of_line->cost        = start_of_line->cost + (pe - start_of_line->p).norm() - fil.adj;
          start_of_line->connects2wp = true;
          start_of_line->children.push_back(pe_node);
          most_recent_node_          = pe_node;
          return true;
...
int RRT::developTree(unsigned int i)
{
  ...
  while (added_new_node == false)
  {
    ...
    added_new_node     = checkForCollision(closest_node, test_point, i);
  }
...
{
  float distance = INFINTIY;
  node* closest_gchild, closest_node;
  for (unsigned int j = 0; j < root->children.size(); j++)
    for (unsigned int k = 0; k < root->children[j]->children.size(); k++)
    {
      d_gchild      = (p - root->children[j]->children[k]->p).norm();
...
bool RRT::checkForCollision(node* ps, NED_s pe, i)
{
  node* start_of_line;
  if (ps->dontConnect) // then try one of the grand children
    start_of_line = findClosestNodeGChild(node* ps, pe->p);
